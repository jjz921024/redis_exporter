...
func (e *Exporter) extractSlowLogMetrics(ch chan<- prometheus.Metric, c redis.Conn) {
	...
	if reply, err := redis.Int64(doRedisCmd(c, "SLOWLOG", "LEN")); err == nil {
		e.registerConstMetricGauge(ch, "slowlog_length", float64(reply), opt.Partition, opt.Host)
	}
	...
	e.registerConstMetricGauge(ch, "slowlog_last_id", float64(slowlogLastID), opt.Partition, opt.Host)
	e.registerConstMetricGauge(ch, "last_slow_execution_duration_seconds", lastSlowExecutionDurationSeconds, opt.Partition, opt.Host)
}
